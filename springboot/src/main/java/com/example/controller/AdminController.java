...
public class AdminController {
    ...
    @PostMapping("/add")
    public Result add(@RequestBody Admin admin) { // @RequestBody 接受前端传来的 json 数据
        adminService.add(admin);
        return Result.success();
    }

    @PutMapping("/update")
    public Result update(@RequestBody Admin admin) { // @RequestBody 接受前端传来的 json 数据
    ...
    public Result deleteBatch(@RequestBody List<Admin> list) { // @RequestBody 接受前端传来的 json 数据
    ...
    public void exportData(Admin admin, HttpServletResponse response) throws Exception {
        ...
        // 默认的,未添加alias的属性也会写出,如果想只写了别名的字段,可以调用此方法排除之
