...
const handleSelectionChange = (rows) => { // rows 就是实际选择的数组
  data.rows = rows;
  data.ids = data.rows.map(v => v.id) // map可以把对象的数组转换成纯数字的数组
}
...
const exportData = () => {
  let idStr = data.ids.join(",") // 把数组转换成字符串 [1,2,3] -> "1,2,3"
  let url = `http://localhost:8088/admin/export?username=${data.username === null ? '' : data.username}`
