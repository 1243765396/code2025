<template>
  <div>
    ...
    <div class="card" style="margin-bottom: 5px">
      <el-input clearable @clear="load" style="width: 240px; margin-right: 5px" v-model="data.userName" placeholder="请输入借阅人" :prefix-icon="Search"></el-input>
    ...
    <div class="card" style="margin-bottom: 5px">
      <el-table :data="data.tableData" style="width: 100%"
                :header-cell-style="{ color: '#333', backgroundColor: '#56e0fc'}">
        <el-table-column label="图书封面">
          <template #default="scope">
            <el-image v-if="scope.row.bookImg" :src="scope.row.bookImg" :preview-src-list="[scope.row.bookImg]" :preview-teleported="true" style="width: 40px; height: 40px; border-radius: 5px; display: block" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="100px" v-if="data.user.role === 'ADMIN'">
          <template #default="scope">
    ...
    <el-dialog title="审核信息" v-model="data.formVisible" width="30%" destroy-on-close>
      <el-form ref="formRef" :model="data.form" label-width="200px" style="padding: 20px 30px 10px 0">
        ...
        <el-form-item prop="reason" label="请假说明" v-if="data.user.role === 'ADMIN' && data.form.status === '审核拒绝'">
          <el-input v-model="data.form.reason" autocomplete="off" placeholder="请输入请假说明" />
        </el-form-item>
      </el-form>
      <template #footer>
